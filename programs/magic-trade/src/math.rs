...
use {crate::error::PlatformError, anchor_lang::prelude::*, std::fmt::Display};
    ...
    T: num_traits::PrimInt + Display,
{
    if let Some(res) = arg1.checked_add(&arg2) {
        ...
        err!(PlatformError::MathOverflow)
    }
}
...
pub fn checked_decimal_div(
    ...
    if coefficient2 == 0 {
        ...
        return err!(PlatformError::MathOverflow);
    }
    if coefficient1 == 0 {
        return Ok(0);
    }
...
pub fn checked_as_u32<T>(arg: T) -> Result<u32>
where
    T: Display + num_traits::ToPrimitive + Clone,
{
    let option: Option<u32> = num_traits::NumCast::from(arg.clone());
    if let Some(res) = option {
        Ok(res)
    } else {
        msg!("Error: Overflow in {} as u32", arg);
        err!(PlatformError::MathOverflow)
    }
}
...
pub fn scale_u128_to_exponent(arg: u128, exponent: i32, target_exponent: i32) -> Result<u128> {
    if target_exponent == exponent {
        return Ok(arg);
    }
    let delta = checked_sub(target_exponent, exponent)?;
